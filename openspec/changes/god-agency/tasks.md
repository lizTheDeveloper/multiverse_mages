...
- [x] 1.3 Amend `contracts.md` §7 with the `worshipSnowball` threshold and the new `ascensionRateByPath` and `favorWasted` metrics
...
- [x] 6.4 Implement the four stock channels — starting favor, materials, populace, and a seeded lootable and burnable archive — each capped by the head-start fraction of the reference-tick baseline
...
- [x] 7.1 Implement `worshipSnowball` as the Gini coefficient of favor regeneration at world ticks 120, 600, 1200, and 2400, with the p95-to-median ratio alongside it
- [x] 7.2 Report per-class saturated worship contributions with the metric so a runaway is attributable without a second sweep
- [x] 7.3 Implement `ascensionRate`, `ascensionRateByPath`, and the three terminal-outcome counts
- [x] 7.4 Implement `prestigeAdvantage` as a high-prestige-versus-fresh sweep
