...
- [ ] 7.5 Record the ambiguities this change resolved against `docs/design/contracts.md` — the `rediscoveryAffinity` direction and the 3× floor, library-instance location, whose tradition populates `preparedSpells`, teaching loss below `fp(1024)`, whether legality gates acquisition, and the persisted ever-known record — and update that document or confirm the resolutions stand
