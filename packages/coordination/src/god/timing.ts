...
export function barIsPlaying(
  ...
  if (subdivision === SUBDIVISION.economy) {
    ...
    //
    // **Reads `material-stock`, not `universe.materials`.** W21 was authored
    // against world-schema revision 4, where the economy was one scalar on the
    // universe row; revision 5 (`city-and-supply-chain`) split it into three
    // kinds and *removed* the field, so the original expression does not compile
    // on this tree and would have been a runtime read of a column that no longer
    // exists. Any of the three being non-empty is a storeroom with something in
    // it, which is what the sentence above says.
    const stock = componentOf(state, MATERIAL_STOCK);
    return (
      stock.get(universe, 'food') > 0 ||
      stock.get(universe, 'stone') > 0 ||
      stock.get(universe, 'vellum') > 0
    );
  }
