...
import { interventionPhase, timedCost } from '../../src/god/timing.js';
...
/** As the sibling god suites build it: nothing here keys a draw on a tick. */
const TEST_RNG = {
  rootSeed: 1,
...
function bench(): Bench {
  ...
  const deps: InterventionDeps = {
    ...
    // The three fields `w109/alliances` added to `InterventionDeps` after this
    // file was written. Inert: nothing in this suite invites a scholar.
...
describe('unmaking an axis change a raid made', () => {
  ...
  it('discharges the mark, so the surcharge is paid exactly once', () => {
    ...
    //
    // **And through §5.2's unease, which is what the `integration/group-e` merge
    // added to this expression.** `w21/timing-and-envelopes` prices every
    // constitutional act by how much of the previous act's unease is still
    // ringing, and the permit two lines up *is* a constitutional act — so the
    // forbid that follows it on the same tick pays the full eight bars. The
    // literal was 16,384 and the merged build charges 49,152, exactly 3x, which
    // is `FP_ONE + 8 x uneaseStep` with `uneaseStep = fp(0.25)`.
    //
    // Composed rather than re-pinned: the surcharge this test is about is
    // discharged (the mark is gone, asserted above), and what is left is the
    // ordinary hysteresis price seen through the unease multiplier. Writing
    // 49,152 as a literal would have hidden the second wire inside a number.
    const before = favorOf(b);
    const phase = interventionPhase(b.state, b.universe, ACTION.forbidTechnique, 0, C);
