...
describe('an absent row is never lit, which is not the same as expired', () => {
  it('restores a pre-revision-12 save with nothing standing and nothing lapsing', () => {
    ...
    // The fixture is the claim: this really is a revision-11 envelope, not a
    // revision-12 one with a name filtered out of the assertion.
    expect(worldSchemaVersionOf(older)).toBe(11);
