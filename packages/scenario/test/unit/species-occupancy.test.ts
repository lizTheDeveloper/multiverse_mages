...
describe('twenty world years in', () => {
  ...
  it('has nothing at the ruleset ceiling any more, and one v1 cell empty', () => {
    ...
    //
    // **Re-measured 2026-08-16 on the Group D integration branch, at the merge
    // of `w191/anti-requisites-in-v1`, and the branch's own six numbers do not
    // survive it**: `11/11/11/9/9/7` on the branch, `11/11/10/11/11/8` here.
    // The branch measured against a tree holding the anti-requisite and nothing
    // else; this one also holds `w35/permit-cost`'s stewardship drain and
    // `w77/effect-displacement`'s displacement term, and both allocate work in
    // the tick, so the handle-keyed re-roll this block's earlier entries
    // describe applies on top of the mechanic.
    //
    // What survives the re-measurement is the *reading*, and it is the half the
    // branch was making a claim about: `perdo-nomen` is still emptied entirely
    // — asserted in its own test below, and it passes here unchanged. The six
    // counts moved; the mechanic did not.
    expect(bySpecies('dwarf').occupiedCells).toBe(11);
    expect(bySpecies('human').occupiedCells).toBe(11);
    expect(bySpecies('orc').occupiedCells).toBe(10);
  ...
  it('measures a spread that is neither flat nor a hegemony', () => {
    ...
    //
    // **0.0484 re-measured 2026-08-16 on the Group D integration branch.** The
    // branch's 0.0805 was taken with the anti-requisite alone; with W35's
    // stewardship drain and W77's displacement also in the tree the spread
    // narrows again, close to where it was before either change. Pinned to four
    // places for the reason the entry above gives.
    expect((entry as { value: number }).value).toBeCloseTo(0.0484, 4);
