...
describe('the order stays total and deterministic', () => {
  ...
  it('every term is ablatable, and ablating one leaves the other five', () => {
    const candidate = target(10, 3, 8192, {
      cellId: cell('rego-terram'),
      formId: form('terram'),
...
describe('the weight table is read from content, never from a literal', () => {
  ...
  it('the role bound cannot outvote the other five terms at their extremes', () => {
    const others = TARGET_TERM_KINDS.filter((kind) => kind !== 'role').reduce(
      (sum, kind) => sum + weights.bound[kind],
      0,
    );
    expect(weights.bound.role).toBeLessThan(others);
  });
});
