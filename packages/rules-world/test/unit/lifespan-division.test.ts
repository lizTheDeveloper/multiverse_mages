...
const ALLOWED: Readonly<Record<string, string>> = {
  'packages/rules-world/src/populace/buckets.ts::normalizedCohortAge':
    'A ratio, not a per-tick rate. The numerator is pre-multiplied by FP_ONE before the division, ' +
    'which is the same extended-scale shape perLifespanRate uses, so it cannot floor to zero ' +
    'forever: a draconic cohort reads 0 at one month, 512 at half its lifespan and 1024 at the ' +
    'end. The cost is granularity — one step of normalized age is ~17.6 months for a dragon — and ' +
    'that is a resolution question for the hazard table, not the immortality defect. Verified ' +
    'numerically across the three longest-lived species before allowlisting.',
  'packages/rules-world/src/populace/mortality.ts::cohortDeathsThisTick':
    'Divides by LIFESPAN_RATE_ONE, the extended-scale constant, not by a lifespan — it narrows a ' +
    'rate that perLifespanRate already computed correctly. Same false positive as narrowToFixed ' +
    'above, and kept as an entry for the same reason: narrowing back to fp is precisely where the ' +
    'defect would be reintroduced, so a reader should find this named rather than filtered out.',
