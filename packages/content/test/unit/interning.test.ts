...
describe('contentRevision', () => {
  ...
  it('is pinned to the digest of the shipped content set', () => {
    ...
    //
    // d4e3047657b4fa8a1a74e1d52f9f5c86 -> 0baf46442bad237948c858c2122d096a, when W35 added the four
    // `stewardship-*` magnitudes to `god-constant.json` — the first drain in the
    // economy, the recurring favor cost of holding a permissive ruleset open. A
    // digest change on a god constant is exactly what the preimage is for: two
    // universes whose rulesets cost different amounts to govern are not running
    // the same game, however identical their node graphs look.
    expect(registry.contentRevision).toBe('0baf46442bad237948c858c2122d096a');
  });
