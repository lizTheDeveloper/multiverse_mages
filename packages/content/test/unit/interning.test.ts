...
describe('contentRevision', () => {
  ...
  it('is pinned to the digest of the shipped content set', () => {
    ...
    // 8681bf84... -> 5c771725..., when `target` scope became a priced axis and
    ...
    //
    // fd64f29d... was this branch's own first value, before its `self` gloss
    // took the measured tier-by-tier ratio. Recorded here because a gloss is in
    // the preimage — the digest is over every interned field, not over the ones
    // the rules read — and the next person to edit a gloss will want to know
    // that before they assume a revision move means a behaviour move.
    expect(registry.contentRevision).toBe('5c7717257b67dd78b59a48240fedbdec');
  });
