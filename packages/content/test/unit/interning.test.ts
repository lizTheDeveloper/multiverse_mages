...
describe('contentRevision', () => {
  ...
  it('is pinned to the digest of the shipped content set', () => {
    ...
    // `main`'s chain met `w28/magic-regimes`' four regimes and the store kind
    // the vocabulary was missing. In the preimage for the reason every tradition
    // field is: a regime decides how a mage holds what she knows, so two
    // universes disagreeing about the set of regimes would arbitrate a raid
    // between two different games while their revisions called them compatible.
    //
    // Neither side's literal is a digest over a preimage holding both, so the
    // value below was **re-measured from the merged tree**.
    expect(registry.contentRevision).toBe('6ba14f8fd27a3218af183a4679eb6d07');
  });
