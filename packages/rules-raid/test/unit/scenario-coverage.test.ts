/*
 * Multiverse Mages — task 10.5 (raid-engagement): every capability-spec
 * scenario is mapped to a test, and the mapping cannot rot.
...
/**
 * Task 10.5 asks someone to *confirm* every scenario across the five
 * raid-engagement capability specs has a corresponding passing test.
 *
 * The confirmation is a committed manifest — `scenario-coverage.md` beside
...
const SPEC_DIR = 'openspec/changes/raid-engagement/specs';
const MANIFEST_PATH = 'openspec/changes/raid-engagement/scenario-coverage.md';

/** The five capability specs this change adds. Read from disk, not listed. */
...
describe('the coverage manifest is a manifest of the specs that exist', () => {
  it('found the five capability specs', () => {
    expect(specNames).toEqual([
      'host-ruleset-arbitration',
      'portals',
      'raid-consequences',
      'raid-objectives',
      'raid-space',
