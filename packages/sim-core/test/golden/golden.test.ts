...
import { DEFAULT_FIXTURES_DIR, fixtureFiles, readFixture, verifyFixture } from './harness.js';

/**
 * This suite calls the harness with **no options**, so the committed gate runs
 * against the committed fixtures and the real world registry. The seams the
 * harness exposes exist for the failing-path tests in `golden-failure.test.ts`,
 * `golden-immutability.test.ts`, and `golden-regeneration.test.ts`; a bug in
 * those seams cannot make this file pass.
 */
...
describe('regeneration is a separate command', () => {
   ...
   *
   * In a green repository no fixture here is failing, which is why
   * `golden-immutability.test.ts` runs the same guarantee against a temporary
   * directory in which one fixture *is* failing. This test says the committed
   * files survive the real suite; that one says they would survive a real
   * failure.
   */
  it('leaves every fixture file byte-identical after the suite has run them', () => {
    const before = new Map(
      files.map((file) => [file, readFileSync(join(DEFAULT_FIXTURES_DIR, file))]),
    );
    ...
    for (const file of files) {
      const after = readFileSync(join(DEFAULT_FIXTURES_DIR, file));
