...
- [ ] 2.5 The mageless/stagnation terminal path: `magelessTicks`, `stagnation-mageless-ticks`, `terminalReason` enum
- [ ] 2.6 The prestige constants and the loader identity asserted over them
- [ ] 2.7 The worship formula and the favor-regeneration loop; `favorWasted`
- [ ] 2.8 Materials production and consumption per world tick
- [ ] 2.9 `capitalSnowball` — formula, threshold, and the §7 metric set around it
- [ ] 2.10 `portalTargets` — its type, and confirmation that nothing populates it
- [ ] 2.11 Whether a universe id stable across simulation instances exists anywhere

### 3. Settle the architecture, not the design

- [ ] 3.1 Decide how a colony is represented without a second resident universe, against `contracts.md` §1.1
- [ ] 3.2 Confirm the representation needs no cross-universe evaluation on unsynchronised clocks (`contracts.md` §0: clocks are per-universe)
- [ ] 3.3 State the `WORLD_SCHEMA_VERSION` revision and the `contentRevision` consequence
- [ ] 3.4 State what the proposal needs from W10 (`pvp-server`) and what it does not
- [ ] 3.5 Confirm no coordinates are introduced anywhere (vision §7a)

### 4. Write the proposal

- [ ] 4.1 Scaffold `openspec/changes/colonization/` and run `openspec validate colonization --strict` on the skeleton, before writing prose
- [ ] 4.2 `proposal.md` — Why / What Changes / Capabilities / Impact, in `raid-engagement`'s house style
- [ ] 4.3 `specs/universe-extinction/spec.md`
- [ ] 4.4 `specs/colonial-tribute/spec.md`
- [ ] 4.5 `specs/multiverse-bubbles/spec.md`
- [ ] 4.6 `design.md` — Context, Goals/Non-Goals, Decisions, the three options with the measurement that would show each failing, Risks, Open Questions
- [ ] 4.7 `tasks.md`
- [ ] 4.8 `openspec validate colonization --strict` green

### 5. The six tensions, each answered explicitly in `design.md`

- [ ] 5.1 **§7a: no map.** Everything transferred — populace, materials, worship — is a count. Bubble membership is an adjacency set, not geography. Say it explicitly; it is the proposal's strongest single argument
- [ ] 5.2 **§8 griefing.** Resolved by the author, not waived: a universe is a run, defeat is a fast re-entry carrying prestige, `prestige-base-stagnated` is non-zero by design. The risk moves to the *conqueror's* side and lands on `capitalSnowball`
- [ ] 5.3 **§8a ascension.** Two win conditions now exist. Raise it as the proposal's central open question; do not resolve it
- [ ] 5.4 **§1.1 one universe per instance.** State the architectural cost precisely and what W10 must carry
- [ ] 5.5 **§12 economy / ranked ladder.** Argue scope rather than assuming it. Split the verdict if the honest answer is split
- [ ] 5.6 **Does it help the measured problem?** 51 of 300 nodes; an idle universe learns all 51. Answer honestly, including "no"

### 6. Name collisions and open questions

- [ ] 6.1 Propose a term for tier promotion that is not `prestige`, and say plainly that the collision exists
- [ ] 6.2 Assemble the author-question list: attribution, colony dynamics, rejoin tier, bubble size and composition, ascension-vs-conquest, prestige across a rejoin, the "special circumstances" unbounded bubble
- [ ] 6.3 Name the measurement that would show each recommendation failing

### 7. Close

- [ ] 7.1 Commit and push `w16/colonization`. No PR
