...
`balance/results-w116-{before,after}-2400.json` for all seven arms, to the reported decimal.
...
2. **The cause is destruction, not a failure to write.** dwarf production *rose*
   (417.1 → 597.3 books). Destruction rose four times as fast (174.6 → 545.5 instances, **4.58 SE**
   — the largest movement in the whole table).
...
4. **What died was redundancy.** `nodesKnown` moved 46.4 → 26.9, **1.09 SE** — inside tolerance —
   and `nodesLost` was zero in the runs inspected. `degradeLibrary` offers duplicates first by
   construction, and `LIBRARY_UPKEEP_PER_INSTANCE`'s docstring says destroying second copies is
   what brake 4 is *for*.
5. **The shelf ceiling is vellum, not `scribeAffinity`.** gnome wrote 113.0 books and kept 75.7;
   ...
   uncorrelated with it. A library's sustainable size is `vellum income ÷ 2 fp per instance` and
   nothing else.
...
- It did not establish *why* elf and draconic point estimates fell; at 20 replicates they are inside
  tolerance and the honest statement is "not measured", not "no effect". Separating them would take
  replicates at roughly `(SE/target)²` — about 180 per arm for elf to resolve a −52 book move at
  3 SE — which is ~15 minutes per arm on the current harness.
