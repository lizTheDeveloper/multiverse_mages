...
**Counts: 6 BUILT · 7 PARTIAL · 13 ABSENT · 4 SUPERSEDED. 30 rows.** One row — the twelve Grungeon
...
| `probable-strategies.md` §4 | *"`knowledgeHalfLife`, `libraryDependence` … are not collected by any sweep, because only the ten `reference*` measures are registered in the scenario."* | **SUPERSEDED** | Registry joined: `REFERENCE_REGISTRIES.metrics` at `packages/scenario/src/sweep.ts:85–93`, with `:80–83` recording that before this *"the validator would have rejected their ids at expansion, which is why `collectRunMetrics` had no production caller."* Collected in production: `executor.ts:57, 546` calls `collectRunMetrics`. **And a committed sweep declares it** — `balance/sweeps/balance-full.sweep.json` (`sweepId: balance-full-v1`) lists `knowledgeHalfLife` and `libraryDependence` among 23 metric ids, and four baselines under `balance/baselines/` carry `provenance.metricDefinitionVersions.knowledgeHalfLife = 1`. The ten-`reference*` sweep the doc describes is the 24-tick shakedown (`REFERENCE_SWEEP … metrics: REFERENCE_METRIC_IDS`, `sweep.ts:155`), which is not the gate. | The doc's stated reason the archivist is unfalsifiable no longer holds. Whether the archivist *is* separated is a different question this audit does not answer; `design-language.md:606` reports the first readings as censored at every horizon — 1 loss in 507 cohort observations at 240 ticks — which is a censored lower bound honestly reported, not a null. |
...
- **`changeTradition`, `RESOLUTION`, `hooksOfTradition`** — pinned, and triaged in
  ...
  expensive* from the price side, and both are true at once. Verified here: the god's dispatch goes
  to a **different** implementation — `packages/coordination/src/god/interventions.ts:336` →
  `traditionPlan` — while `rules-magic/src/traditions/change.ts:changeTradition` is the unreached
  one. And the price arithmetic holds: `god-cost.json` prices `change-tradition` at `favorCost
  65536` against a cap of `favor-cap-base 20480 + tier × favor-cap-per-tier 10240`, which at
  `worship-tier-count 5` tops out at 71,680 — so the action is purchasable at exactly one tier and
  structurally impossible below it, as its own gloss says it was priced to be. **Two documents
  describing one action from opposite ends, neither citing the other**, and a third fact — two
  implementations — that neither records.
