...
|---|---|---|---|---|
| `contracts.md` §1.5 | `mastery` thresholds: `fp(1024)` is full, below `TEACH_THRESHOLD` a mage may not teach, between them teaching transmits "at proportionally reduced mastery", with a strict floor-at-one-unit reduction rule | **PARTIAL** | `rules-magic/src/instances/constants.ts:50` `DEFAULT_INITIAL_MASTERY = 256`, `:63` `DEFAULT_TEACH_THRESHOLD = 512`, `:116` `MASTERY_MAX = 1024`. The **only** production writer of mastery is `instances/decay.ts:213`, inside `decayHeldKnowledge`, and it lowers (`decayedMastery`). Probe control: the same probe finds `subsystem.ts:292`, the setter definition. **The gate is real and reads the threshold** — `coordination/src/gateway.ts:501` (`mastery >= DEFAULT_TEACH_THRESHOLD`) and `:519` (`if (mastery < DEFAULT_TEACH_THRESHOLD) continue`), with `rules-magic/src/instances/teaching.ts:147` defaulting to it and `:186` writing `transmittedMastery(teacherMastery, jitter)`, the strict-reduction arithmetic itself. `grant-mastery` is `1024` in `content/data/god-constant.json`, and its own gloss says a grant at the research default *"would sit below the teach threshold and never leave the founder's head"*. `metric-constants.md:58` states the same thing outright | Research creates instances permanently below the teach threshold, so **no researched node is ever teachable**. The whole `[512, 1024]` interval is reachable only by god action 8 at `grant-mastery = 1024`, and those slide down from the first decay tick. `teachCost`, `teach-rate` and the strict-reduction arithmetic are all specified over an interval nothing occupies |
| `contracts.md` §2.9 | "the loader fails a set that declares one nothing reads, because an unread constant is a tuning knob that does nothing and the sweep that turned it would report the null result as a finding about the game" | **PARTIAL** | The guard is `content/src/god.ts:280–287`, checked against `REQUIRED_GOD_CONSTANTS` at `:61` — a **hardcoded frozen array**. It tests membership of a hand-maintained list, not that anything reads the value. Eight constants sit on that list without a rules-path reader, all pinned in the baseline as `constantsBehindDeadCode` / `unconsumedConstants` (all `coordination/src/god/constants.ts`). **Seven have no reader at all** — `legacyArchiveMaxTier`, `legacyArchiveNodes`, `legacyBaselineFavor`, `legacyBaselineMaterials`, `legacyBaselinePopulace`, `legacyHeadstartFraction`, `legacyReferenceTick` — their only reader being `legacyGrant` (`god/ascension.ts:512`), whose sole non-definition occurrence is the barrel re-export `god/index.ts:109`. **The eighth, `prestigeRetention`, is narrower**: the `prestige-retention` *record* is read at load by §2.9's own identity check, so it is not unread — but the derived `coordination` constant has no rules-path consumer. `god/constants.ts:147` admits the shape in code: *"Provenance, not an input. No formula reads it"* | Exactly the failure §2.9 names, live. Eight authored knobs look turnable to a content author and turn nothing; a sweep varying `legacy-*` would report a null result as a finding about the game. The guard reports on the wrong input and cannot go red |
| `contracts.md` §1.4 / vision §6a; `invariants.md` INV-29 | Library depth feeds research rate — the knowledge-as-capital compounding loop | **PARTIAL** | The loop **is** built: `world-step.ts:724` calls `libraryCapital`, `:1580` reads `capital.depthFor(row.universityId)`, `:1592` applies `libraryRateMultiplier`, and `rules-world/src/universities/capital.ts:250` `emitCapital` emits `research-rate`/`teach-rate`/`scribe-rate`. It is keyed on the mage's `universityId`. The **only** production write of a non-zero `universityId` is `scenario/src/reference-universe.ts:695`, the founding cohort — a real handle, with a real library, created at `:637–640`. `world-step.ts:1807` creates run-born mages with no university; `:1431` zeroes it on death. The only path that could affiliate one is `changeAffiliation` (`rules-world/src/mages/roles.ts:95`), reached only by `completeAffiliation` (`autonomy/affiliation.ts:58`) — **both pinned in the baseline**, the latter `unreached`. `world-step.ts:1540` mentions it in a comment only | **No mage created after tick 0 can ever be affiliated**, so the loop's input is confined to the founding cohort and is gone once it dies. Over a 200-year reference run §6a's loop is asymptotically off, INV-29 has almost nothing to measure past the first lifespan, and `hard-magic.md`'s flat 51-node plateau across strategies is consistent with it. Universities are founded (`god/interventions.ts:781`), charged upkeep (`world-step.ts:748`) and staffed by nobody. *Not measured here:* whether `depthFor` ever returns a non-neutral depth for the founders — that needs a run, and the audit took none |
...
`grant-mastery`, which `content/data/god-constant.json` sets to `1024`, and those instances start
sliding on the next decay tick.

The gate is not decorative — it is the load-bearing half. `coordination/src/gateway.ts:501` returns
`mastery >= DEFAULT_TEACH_THRESHOLD` and `:519` skips any teacher below it, so the comparison is made
on the live teaching path; and when a lesson does happen, `rules-magic/src/instances/teaching.ts:186`
writes `transmittedMastery(teacherMastery, jitter)`, which is §1.5's strict-reduction arithmetic
exactly as specified. Every piece works. The interval it operates on is the part that is empty.
`grant-mastery`'s own gloss is the clearest statement of the problem anywhere in the repository: *"A
grant at the research default would sit below the teach threshold and never leave the founder's
head."* That sentence is true of every instance the universe produces for itself.
...
The founding cohort is genuinely affiliated — `reference-universe.ts:637–640` creates a real library
and a real university and binds the handle the mages are given — so the loop's input is not zero from
the start. It is confined to a cohort that dies. What is certain, and is the narrow claim worth
acting on, is that **no mage created after tick 0 can ever be affiliated**. Whether `depthFor` ever
returns a non-neutral depth even for the founders needs a run to answer, and this audit took none.
...
whose sole non-definition occurrence in the tree is a barrel re-export at `god/index.ts:109`. Seven
have no reader at all; the eighth, `prestige-retention`, is read by §2.9's own load-time identity
check and by nothing in the rules path. The code says so itself at `god/constants.ts:147`:
*"Provenance, not an input. No formula reads it."*
...
of two implementations is canonical. Removing the two takes the triage's §2 count of 46 load-bearing
findings to **44**, and its §3 superseded count from 14 to **16**. `changeTradition` stays in §2: it
is a real `PARTIAL`, not a false gap, and the row below narrows it rather than removing it.
