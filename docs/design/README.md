...
| 16 | [`reachability-triage.md`](reachability-triage.md) | **measured** | The 124 findings of `npm run check:reachability`, at `e2b89d8` (2026-08-14, amended on `w190/scribing-fidelity` when `STANDARD_STORE` was repaired): 60 integration-debt items, 14 superseded accessors, 27 tooling-only exports, 23 dead symbols, 0 false positives. An inventory, not a fix. §2 names the mechanisms whose absence disables a subsystem; §3 and §5 are why an unreached symbol is never by itself a claim about a capability. |
